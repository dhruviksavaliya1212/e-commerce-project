...
const Desc = ({ addToCart, removeFromCart, cartItem, itemsData, setItemsData, url, item_list }) => {
  const { id } = useParams();
        ...
        <div data-aos="fade-up" data-aos-delay="400" data-aos-duration="500" className=" w-[95%] flex flex-col lg:flex-row justify-center items-center gap-0 lg:gap-10">
          ...
          <div className=" m-5 flex flex-col gap-2">
            ...
            <div className=" w-[300px] flex flex-col gap-1">
              <p className={ itemsData.color === "" ? "hidden":" font-medium text-sm grid grid-cols-2  "}>
                Colors
                <span className=" capitalize text-zinc-950 ">
                  {itemsData.color}
                </span>
              </p>
              <p className={ itemsData.size === "" ? "hidden":" font-medium text-sm grid grid-cols-2  "}>
                RAM & ROM
                <span className=" capitalize text-zinc-950 ">
                  {itemsData.size}
                </span>
              </p>
              <p className={ itemsData.brand === "" ? "hidden":" font-medium text-sm grid grid-cols-2  "}>
                Brand
                <span className=" capitalize text-zinc-800 ">
                  {itemsData.brand}
                </span>
              </p>
              <p className={ itemsData.os === "" ? "hidden":" font-medium text-sm grid grid-cols-2  "}>
                OS
                <span className=" capitalize text-zinc-950 ">
                  {itemsData.os}
                </span>
              </p>
              <p className={ itemsData.screensize === "" ? "hidden":" font-medium text-sm grid grid-cols-2  "}>
                Screen Size
                <span className=" capitalize text-zinc-950 ">
                  {itemsData.screensize}
                </span>
              </p>
              <p className={ itemsData.cpu === "" ? "hidden":" font-medium text-sm grid grid-cols-2  "}>
                CPU
                <span className=" capitalize text-zinc-950 ">
                  {itemsData.cpu}
                </span>
              </p>
              <p className={ itemsData.display === "" ? "hidden":" font-medium text-sm grid grid-cols-2  "}>
                Display
                <span className=" capitalize text-zinc-950 ">
                  {itemsData.display}
                </span>
              </p>
              <p className={ itemsData.resolution === "" ? "hidden":" font-medium text-sm grid grid-cols-2  "}>
                Resolution
                <span className=" capitalize text-zinc-950 ">
                  {itemsData.resolution}
                </span>
              </p>
              <p className={ itemsData.refreshrate === "" ? "hidden":" font-medium text-sm grid grid-cols-2  "}>
                Refreshrate
                <span className=" capitalize text-zinc-950 ">
                  {itemsData.refreshrate}
                </span>
              </p>
              <p className={ itemsData.earplacement === "" ? "hidden":" font-medium text-sm grid grid-cols-2  "}>
                Ear Placements
                <span className=" capitalize text-zinc-950 ">
                  {itemsData.earplacement}
                </span>
              </p>
              <p className={ itemsData.formfactor === "" ? "hidden":" font-medium text-sm grid grid-cols-2  "}>
                Form Factor
                <span className=" capitalize text-zinc-950 ">
